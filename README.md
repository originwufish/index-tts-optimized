# IndexTTS Optimized
IndexTTS2 语音合成优化增强版 | 生产级 TTS API 服务
**支持多说话人配音 / 角色切换 / 文本预处理 / 音频归一化**

## 项目介绍
本项目基于 IndexTTS2 构建高性能语音合成服务，在原版基础上优化文本处理、音频质量、服务稳定性，并提供标准化、可直接部署的 FastAPI 接口。
支持多人配音、角色切换、停顿控制、语速/音量/情感调节，适用于智能播报、多角色对话、有声内容生成等场景。

---

## 优化内容
### 1. 文本预处理优化
- 支持数字转中文（电话/数值/序号三种模式）
- 支持自定义拼音标注：汉字[pin1]
- 支持局部替换语法 `[replace:old=new]`
- 自动清理无效连字符、符号
- 支持停顿标签 `[pause:1.0]`

### 2. 音频合成质量优化
- 分段音频自动响度归一化（LUFS）
- 全局音量统一，避免忽大忽小
- 长文本自动切分 + 无缝拼接
- 支持语速、音量、情感强度调节
- 支持目标时长自动调速

### 3. 服务稳定性优化
- 启动时自动校验模型文件完整性
- 完善异常捕获与错误提示
- 自动清理临时文件
- 双端口并发服务（8888 / 8890）
- 同步返回合成音频

### 4. 核心新增：多人配音（角色切换）
- 支持一句话内多个角色无缝切换配音
- 角色音频统一存放：`spk/` 文件夹
- 角色配置文件：`speakers.json`
- 角色切换标签：`[anchor:角色名:字数]`

---

## 新增功能
✅ 多人配音 / 多角色切换
✅ 自定义停顿时长
✅ 数字中文智能读法
✅ 音频响度归一化
✅ 语速/音量/情感调节
✅ 目标时长自动匹配
✅ 健康检查接口
✅ 监控指标接口

---

## 环境依赖
```
fastapi
uvicorn
pydantic
python-multipart
cn2an
```

**系统依赖**
- ffmpeg
- ffprobe

---

## 快速启动
```bash
pip install -r requirements.txt
python app.py
```

服务端口：**8888 / 8890**
API 文档：`http://localhost:8888/docs`

---

# 🧑‍🧑‍🧒 多人配音（角色切换）使用说明
## 1. 文件夹结构
```
项目根目录/
├── spk/              <-- 存放所有角色参考音频
│   ├── speaker1.wav
│   ├── speaker2.wav
│   ├── teacher.wav
│   └── robot.wav
├── speakers.json      <-- 角色配置文件
└── app.py
```

## 2. 角色音频存放
将每个角色的参考音色音频（wav 格式）放入 **`spk/` 文件夹**

## 3. speakers.json 配置（直接复制可用）
```json
{
    "speaker1": "spk/speaker1.wav",
    "speaker2": "spk/speaker2.wav",
    "teacher": "spk/teacher.wav",
    "robot": "spk/robot.wav",
    "xiaomei": "spk/xiaomei.wav",
    "xiaonan": "spk/xiaonan.wav"
}
```

## 4. 多人配音标志位语法
```
[anchor:角色名:字数]
```
- **角色名**：必须与 `speakers.json` 中的 key 一致
- **字数**：从当前位置开始，后续多少个字使用该角色配音

### 使用示例
```
大家好，我是二号角色。[anchor:speaker1:10]我是主持人
```

---

# 📌 Postman 接口使用示例（同步返回音频）

## 1. 创建语音合成任务
### 请求信息
- **URL**
  ```
  http://localhost:8888/api/v1/tts/tasks
  ```
- **Method**：`POST`
- **Content-Type**：`multipart/form-data`

### 请求参数
| 参数名 | 类型 | 必填 | 示例值 |
| --- | --- | --- | --- |
| text | string | 是 | 大家好，我是二号角色。[anchor:speaker1:10]我是主持人 |
| return_audio | bool | 是 | true |
| speed | float | 否 | 1.0 |
| volume | float | 否 | 1.0 |
| emo_weight | float | 否 | 0.65 |
| prompt_audio | file | 是 | 上传wav格式参考音频 |
| target_duration_sec | float | 否 | 10 |

### 可直接复制的测试文本
```
大家好，我是二号角色。[anchor:speaker1:10]我是主持人
```

### 成功返回
```json
{
    "output_audio_path": "/outputs/tasks/a7e40e5c-fd8d-40f6-b2d3-4224106e3093.wav",
    "status": 200
}
```

---

## 2. 健康检查
```
GET http://localhost:8888/health
```

### 返回
```json
{
    "status": "ok"
}
```

---

# 支持的标签语法
- 停顿：`[pause:0.5]`
- 角色切换：`[anchor:speaker1:6]`
- 拼音：`好[hao3]`
- 电话数字：`13800138000[phone:11]`
- 文本替换：`[replace:old=new]`

---

# 未来优化方向
## 1. 标志位功能增强
- **局部音量调节**：新增标签 `[volume:数值:字数]`（数值范围 0.1-2.0，控制后续指定字数的音量大小，1.0 为默认音量）
- **局部语速调节**：新增标签 `[speed:数值:字数]`（数值范围 0.5-2.0，控制后续指定字数的语速快慢，1.0 为默认语速）
- **局部情感调节**：新增标签 `[emo:类型:字数]`（情感类型支持 happy/angry/sad/calm，控制后续指定字数的情感基调）

### 扩展标志位语法说明
| 标签类型       | 语法格式                  | 说明                                                                 | 示例                          |
|----------------|---------------------------|----------------------------------------------------------------------|-------------------------------|
| 局部音量调节   | `[volume:数值:字数]`      | 数值范围 0.1-2.0，1.0 为默认音量                                      | `[volume:1.5:8]` 提高后续8字音量 |
| 局部语速调节   | `[speed:数值:字数]`       | 数值范围 0.5-2.0，1.0 为默认语速                                      | `[speed:0.8:10]` 放慢后续10字语速 |
| 局部情感调节   | `[emo:类型:字数]`         | 情感类型：happy（开心）/ angry（愤怒）/ sad（悲伤）/ calm（平静）       | `[emo:happy:6]` 后续6字用开心语气 |

### 组合使用示例
```
[anchor:teacher:12][volume:1.2:12][speed:0.9:12]同学们注意听讲，[emo:angry:5]不要交头接耳
```
（教师角色，后续12字提高音量、放慢语速；其中最后5字使用愤怒情感基调）

## 2. 性能优化
- **模型格式转换**：将原生模型转为 ONNX 格式，提升跨平台兼容性与推理效率
- **编程语言适配**：支持 Rust/C++ 二次开发，满足高性能场景需求
- **推理框架集成**：接入 vLLM 推理框架，优化批量请求处理速度，降低推理延迟
